...
- Uses `gt rig add <name> <git-url> --local-repo <path>` so Camp Leatherneck creates a fresh,
...
- Leaves the resulting rig with the normal `.repo.git`, `mayor/rig`, `refinery/rig`,
  `settings/`, and `.beads/` layout that Camp Leatherneck expects.
- Optionally pins per-rig role agents in `settings/config.json`.

When to still use `--adopt`:

- You already have a real Camp Leatherneck rig directory that was created elsewhere and you
