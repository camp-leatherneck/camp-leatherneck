...
func CommitWorkingSet(dbDir string) error {
	...
	commitCmd := exec.Command("dolt", "commit", "-m", "lt dolt sync: auto-commit working changes")
	commitCmd.Dir = dbDir
...
func PushDatabaseSQL(townRoot, db, remote string, force bool) error {
	...
	commitQuery := fmt.Sprintf(
		"USE `%s`; CALL DOLT_COMMIT('-m', 'lt dolt sync: auto-commit working changes', '--allow-empty', '--author', 'Camp Leatherneck Sync <sync@gastown.local>')",
		db,
	)
