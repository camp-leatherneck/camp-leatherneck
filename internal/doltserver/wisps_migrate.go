...
// the wisps tables exist on the lt Dolt server (port 3307), since the reaper connects
...
func MigrateAgentBeadsToWisps(townRoot, workDir string, dryRun bool) (*MigrateWispsResult, error) {
	...
	// Step 3b: Ensure wisps tables also exist on the lt Dolt server.
	// The reaper connects directly to the lt Dolt server (port 3307), which is
	...
	if dbName != "" {
		...
		if err != nil {
			fmt.Printf("  Note: lt Dolt server table creation failed: %v\n", err)
		} else {
			if gtCreated {
				result.WispsTableCreated = true
				fmt.Printf("  ✓ Created wisps table on lt Dolt server (%s:%d/%s)\n", gtConfig.Host, gtConfig.Port, dbName)
			}
			for _, t := range gtAux {
				result.AuxTablesCreated = append(result.AuxTablesCreated, t+" (lt server)")
				fmt.Printf("  ✓ Created %s on lt Dolt server\n", t)
			}
		}
	}
...
// For the HQ root itself, returns "hq". For rig directories, returns the rig name.
...
// lt Dolt server via MySQL protocol. This is needed because the reaper connects
...
func ensureWispsOnGTServer(host string, port int, dbName string) (wispsCreated bool, auxCreated []string, err error) {
	...
	if err != nil {
		return false, nil, fmt.Errorf("connect to lt Dolt server: %w", err)
	}
	defer db.Close()
...
// gtTableExists checks if a table exists on the lt Dolt server.
