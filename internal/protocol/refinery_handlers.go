...
// convoy, the Refinery skips processing. These MRs shouldn't exist (lt done
// skips MR creation for owned+direct), but this guards against edge cases.
//
// NOTE: The merge-request bead is created by `lt done`, so we no longer need
...
func (h *DefaultRefineryHandler) HandleMergeReady(payload *MergeReadyPayload) error {
	...
	// The merge-request bead is created by `lt done` with gt:merge-request label.
