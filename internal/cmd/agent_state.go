...
OPERATIONS:
  Get all labels (default):
    lt agent state <agent-bead>

  Set a label:
    lt agent state <agent-bead> --set idle=0
    lt agent state <agent-bead> --set idle=0 --set backoff=30s

  Increment a numeric label:
    lt agent state <agent-bead> --incr idle
    (Creates label with value 1 if not present)

  Delete a label:
    lt agent state <agent-bead> --del idle
...
EXAMPLES:
  ...
  lt agent state gt-gastown-witness

  # Reset idle counter after finding work
  lt agent state gt-gastown-witness --set idle=0

  # Increment idle counter on timeout
  lt agent state gt-gastown-witness --incr idle

  # Get state as JSON
  lt agent state gt-gastown-witness --json`,
