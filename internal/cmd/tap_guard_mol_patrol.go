...
'lt mol patrol' terminates stale molecules. Running it from within an
agent session risks killing sibling agents or even the caller's own
molecule.

This guard blocks:
  - lt mol patrol (when called from within a Camp Leatherneck agent)

Exit codes:
  0 - Operation allowed (not in Camp Leatherneck agent context, or Mayor)
  2 - Operation BLOCKED (in agent context)

Mol patrol should only be run by the Mayor or by humans from outside
the Camp Leatherneck agent tree.`,
...
func runTapGuardMolPatrol(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintln(os.Stderr, "╠══════════════════════════════════════════════════════════════════╣")
	fmt.Fprintln(os.Stderr, "║  Running 'lt mol patrol' from an agent can kill sibling agents  ║")
	...
	fmt.Fprintln(os.Stderr, "║    lt mol status    (safe, read-only)                           ║")
	fmt.Fprintln(os.Stderr, "╚══════════════════════════════════════════════════════════════════╝")
	fmt.Fprintln(os.Stderr, "")
