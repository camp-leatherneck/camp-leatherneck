...
//
// These tests ensure Claude Code hook configurations are correct across Camp Leatherneck.
// Specifically, they validate that:
// - All SessionStart hooks with `lt prime` include the `--hook` flag
...
// findTownRoot walks up from cwd to find the Camp Leatherneck root.
...
func findTownRoot() (string, error) {
	...
	for {
		// hooks/registry.toml is unique to the HQ root
...
// `lt prime` include the `--hook` flag. Without this flag, sessions won't
...
func TestSessionStartHooksHaveHookFlag(t *testing.T) {
	...
	if err != nil {
		t.Skip("Not running inside Camp Leatherneck directory structure")
	}
	...
	for _, path := range settingsFiles {
		...
		for _, entry := range sessionStartHooks {
			for _, hook := range entry.Hooks {
				cmd := hook.Command
				// Check if command contains "lt prime" but not "--hook"
				if strings.Contains(cmd, "lt prime") && !strings.Contains(cmd, "--hook") {
	...
	if len(failures) > 0 {
		t.Errorf("SessionStart hooks missing --hook flag in lt prime command:\n  %s\n\n"+
			"The --hook flag is required for seance to discover predecessor sessions.\n"+
			"Fix by changing 'lt prime' to 'lt prime --hook' in these files.",
...
// settings files in the town, any SessionStart hook containing "lt prime"
...
func TestAllAgentSessionStartHooksHaveHookFlag(t *testing.T) {
	...
	for _, pattern := range knownAgentPatterns {
		...
		err := filepath.Walk(townRoot, func(path string, info os.FileInfo, err error) error {
			...
			for _, entry := range sessionStartHooks {
				for _, hook := range entry.Hooks {
					if strings.Contains(hook.Command, "lt prime") && !strings.Contains(hook.Command, "--hook") {
						relPath, _ := filepath.Rel(townRoot, path)
						failures = append(failures, relPath)
					}
				}
